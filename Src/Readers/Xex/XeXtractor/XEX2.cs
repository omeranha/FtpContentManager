...
namespace XeXtractor
{
	public class XEX2
	{
		...
		public EndianIo Readers { get; private set; }
		...
		{
			Readers = new EndianIo(data, EndianType.BigEndian);
		}
		public XEX2(EndianIo io) { Readers = io; }
		public XEX2(string filePath)
		{
			Readers = new EndianIo(filePath, EndianType.BigEndian);
		}

		public void Open() { Readers.Open(FileMode.Open); }
		public void Close() { Readers.Close(); }
		...
		public void Read()
		{
			...
			{
				ImageKeys key = (ImageKeys)Readers.In.ReadInt32();
				OptionalHeader optHeader = GetOptionalHeader(key, Readers.In.ReadInt32());
				OptionalHeaders.Add(key, optHeader);
			}

			// Read our security info
			Readers.SeekTo(Header.SecurityInfoOffset);
			SecurityInfo.Read(Readers.In);

			// Read our section table
			SectionTable.Read(Readers.In);

			// Go through and read each optional header now
			foreach (var pair in OptionalHeaders)
				pair.Value.Read(Readers.In);
		...
		public void GetXexHeader(Stream outStream)
		{
			// Seek to our header
			Readers.SeekTo(0);
			outStream.Write(Readers.In.ReadBytes(Header.DataOffset), 0, Header.DataOffset);
			outStream.Seek(0, SeekOrigin.Begin);
		}
		...
		public bool DecryptBaseFile(Stream outBaseFile)
		{
			...
			// Seek to our data
			Readers.SeekTo(Header.DataOffset);
			...
			if (format.EncryptionType == BaseFileFormat.EncryptionTypes.Encrypted)
			{
				...
				aes = Rijndael.Create();
				aes.Padding = PaddingMode.None;
				using (CryptoStream decrypt = new CryptoStream(Readers.Stream,
					aes.CreateDecryptor(decryptKey, new byte[0x10]), CryptoStreamMode.Read))
				{
					...
					while (compBlock.DataSize != 0)
					{
						...
						Readers.Stream.Read(testData, 0, compBlock.DataSize);
						Readers.Stream.Seek(compBlock.DataSize*-1, SeekOrigin.Current);
			...
			else
			{
					...
					BaseFileFormat.CompressionTypes.NotCompressed)
				{
					...
					for (int x = 0; x < rawBaseFile.Blocks.Length; x++)
					{
						byte[] buffer = Readers.In.ReadBytes(rawBaseFile.Blocks[x].DataSize);
						outBaseFile.Write(buffer, 0, buffer.Length);
						outBaseFile.Write(new byte[rawBaseFile.Blocks[x].ZeroSize], 0,
								 rawBaseFile.Blocks[x].ZeroSize);
					}
